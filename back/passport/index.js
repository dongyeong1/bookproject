...
module.exports = () => {
  passport.serializeUser((user, done) => { // //passport실행순간(req.login) 실행되면서  서버쪽에 [{ id: 1, cookie: 'clhxy' }] 저장됨 
    done(null, user.id);
  });

  passport.deserializeUser(async (id, done) => {//로그인한다음은 req.user에 사용자 정보가 들어가있음
    try {
